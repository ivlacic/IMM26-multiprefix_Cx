...
# adds (k-1)/(2N) nats to the raw plug-in entropy estimate, then converts to 
# bits by dividing by ln(2). This corrects the downward bias of the plug-in 
# estimator on finite samples.
...
# be tested later.
#
# A row_id is added before pivoting so that group_by() + lead()
# never bleeds across token rows.
#
# pos is sorted numerically to guarantee 
...
#   - base (repeated adjectival bases)
#
...
# p-value is 0.083 (perfect monotone association) -- p-values indicate 
# significance only approximately in this low-n setting.
...
ggsave(
  file.path(plot_dir, "01_prefix_profile.pdf"),
  p1,
  width = 7,
  height = 5.2,
  device = cairo_pdf
...
ggsave(
  file.path(plot_dir, "02_model_fit.pdf"),
  p2,
  width = 11,
  height = 10,
  device = cairo_pdf
...
ggsave(
  file.path(plot_dir, "03_coefficients.pdf"),
  p3,
  width = 7,
  height = 4,
  device = cairo_pdf
...
ggsave(
  file.path(plot_dir, "04_pair_proportions.pdf"),
  p4,
  width = 8,
  height = 6,
  device = cairo_pdf
...
ggsave(
  file.path(plot_dir, "05_adjacency.pdf"),
  p5,
  width = 7,
  height = 5,
  device = cairo_pdf
