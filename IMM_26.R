...
#   6. Plackett-Luce robustness check.
#
...
#   - pair_counts.csv
#   - plots/01_prefix_profile.png
#   - plots/03_coefficients.png
#   - plots/04_pair_proportions.png
#   - plots/05_adjacency.png
#   - hdplot1.pdf
# ============================================================================
...
#
# The Miller-Madow correction adds (k-1)/(2N) nats to the raw plug-in
# entropy estimate, then converts to bits by dividing by ln(2). This
# corrects the well-known downward bias of the plug-in estimator on
# finite samples.
...
ICTRANS <- freq_table %>%
  ...
  mutate(
    # (k-1)/(2N) is the correction in nats; divide by log(2) to convert to bits
...
# Verify TYPE is a unique row identifier before any join or grouping
...
#   - Non-adjacent pairs: outermost vs. innermost position when there are
#                         at least 3 stacked prefixes, i.e. prefix_1 vs.
#                         prefix_3 (for 3-prefix stacks) or prefix_4
#                         (for the single 4-prefix stack).
#
# Each pair is tagged with an `adjacency` column so that its role can
# be tested later.
#
# FIX: a row_id is added before pivoting so that group_by() + lead()
# never bleeds across token rows (previously, TYPE alone was not a
# unique row identifier, causing spurious cross-token pairs).
#
# FIX: pos is sorted numerically to guarantee 
# prefix_1 < prefix_2 < prefix_3 < prefix_4 even if more positions
# were ever added.
...
stacking_pairs_adjacent <- stackings %>%
  ...
  filter(!is.na(prefix)) %>%
  # Sort numerically within each token row so lead() always steps forward
...
# --- 7b. Non-adjacent pairs: prefix_1 vs. last prefix in the stack ---
# For 3-prefix stacks the innermost is prefix_3; for the single
# 4-prefix stack the innermost is prefix_4.
...
stacking_pairs_nonadjacent <- stackings %>%
  ...
  mutate(pos_num = as.integer(gsub("prefix_", "", pos))) %>%
  group_by(row_id, TYPE, base) %>%
  # Keep only rows with at least 3 prefixes and pick outermost / innermost
  filter(n() >= 3) %>%
  summarise(
    prefix_left  = prefix[pos_num == min(pos_num)],   # outermost (prefix_1)
    prefix_right = prefix[pos_num == max(pos_num)],   # innermost (prefix_3 or _4)
    .groups = "drop"
  ) %>%
...
# NOTE: dICTRANS > 0 and dICLOCAL < 0 will look favorable by construction
# because differences are relative to the observed left-right order;
# treat the proportions below as diagnostics, not inferential tests.
...
# order and are trivially inflated. Do not report as inferential results.

cat("WARNING: proportions below are descriptive diagnostics, not hypothesis tests.\n")
...
stacked_model <- stacked_full %>%
  filter(prefix_left != prefix_right) %>%    # drop same-prefix pairs (if any)
  mutate(
...
# NOTE: first() is safe here because dICTRANS / dICLOCAL are constant
# within a pair by construction (they depend only on prefix identity,
# not on individual tokens).
...
#
# NOTE: (1 | pair) is intentionally omitted. dICTRANS and dICLOCAL are
# constant within each pair by construction, so a pair-level random
# intercept would be collinear with the fixed effects and either fail
# to converge or be unidentifiable.
...
# Null model: intercept + base-level random effect
...
if (length(unique(stacked_model$adjacency)) > 1) {
  ...
  # Full interaction: adjacency modulates both slopes
  m_adj_int <- glmer(
    observed ~ (dICTRANS_z + dICLOCAL_z) * adjacency +
      (1 | base),
    data    = stacked_model,
...
# Each pair contributes a single data point, so this is immune to
# within-pair repeated observations -- interpret as a complement to,
# not a substitute for, the mixed model above.
...
#
# Predictions are attached directly to stacked_model (rather than via an
# index-aligned vector) to avoid any risk of row-order mismatch.
...
# ----------------------------------------------------------------------------
# The headline fit statistic (mean |residual| for non-arci pairs) is a
# point estimate. To quantify uncertainty, we bootstrap at the TOKEN level
# within non-arci pairs: each replicate resamples tokens with replacement,
# recomputes each pair's observed proportion and residual, then averages
# |residual| across pairs.
#
# pred is already a column in stacked_model from Section 16.
...
# One bootstrap replicate: resample tokens within each pair independently,
# recompute observed proportion and residual per pair, return mean |residual|.
...
cat(sprintf(
  "\nNon-arci mean |residual| (bootstrap): %.4f\n  95%% CI: [%.4f, %.4f] (median = %.4f)\n  Based on %d bootstrap replicates\n",
  estimate, ci[1], ci[3], ci[2], n_boot
))
...
# ----------------------------------------------------------------------------
# Rows = tokens, columns = prefixes, values = position (1 = leftmost, 0 = absent).
# Tokens with fewer than 2 prefixes are dropped as they contribute no
# ordering information. The two NA rows in the original data are also removed.
...
# Drop rows with fewer than 2 prefixes present
...
# Verify strict ordering (> not =) -- ties indicate a construction error
...
if (any(always_first)) warning("Perfect separation (always first): ",
                               paste(names(which(always_first)), collapse = ", "))
if (any(always_last))  warning("Perfect separation (always last): ",
                               paste(names(which(always_last)),  collapse = ", "))
...
# ----------------------------------------------------------------------------
# Worth is on the multiplicative scale (one prefix fixed at 1 as reference).
# Higher worth = stronger pull toward the leftmost position.
#
# NOTE: with only n = 6 prefixes the minimum achievable two-tailed Spearman
# p-value is 0.083 (perfect monotone association). Report rho and its
# direction as the primary result; p-values indicate significance only
# approximately in this low-n setting.
...
# ----------------------------------------------------------------------------
# Flag prefixes whose worth rank diverges from their ICTRANS and ICLOCAL ranks.
# Large divergence signals a prefix-specific factor not captured by the
# informational model.
# Rank divergence is used (rather than raw residuals) because worth is on a
# multiplicative scale and is not directly comparable to ICTRANS/ICLOCAL units.

worth_df <- worth_df %>%
  mutate(
    rank_worth   = rank(-worth),
    rank_ICTRANS = rank(-ICTRANS_MM),
    rank_ICLOCAL = rank(ICLOCAL),      # higher ICLOCAL = more marked = lower worth expected
    dev_ICTRANS  = rank_worth - rank_ICTRANS,
    dev_ICLOCAL  = rank_worth - rank_ICLOCAL
  )

cat("\n--- Worth rank vs. informational rank deviations ---\n")
print(worth_df %>% select(Prefix, rank_worth, rank_ICTRANS, rank_ICLOCAL,
                          dev_ICTRANS, dev_ICLOCAL))


# ============================================================================
# PLOTS
# ============================================================================
# Five plots covering the main findings:
...
#   5. Adjacency comparison (effect of dICTRANS split by adjacency)
...
# Quick sanity check: display all plots
